...
#Accediendo a los tags de noticias
#Convertimos el json en dataframe a procesar
...
getInfotag<-function(tag,fecha){
  ...
  res<-ifelse(length(tam)>1,  paste0(gsub("+", "", tam[2]),tam[1]),tam) #Comprobamos las comas!
...
insertTag<-function(valor,id_not){
  query<-dbSendQuery(mydb, paste0("insert into tags (id_noticia, visitas)  values ('",id_not,"', '",valor,"');"))
}
